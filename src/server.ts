...
import { streamChatResponse, ChatMessage, HANDOFF_MARKER } from "./chat";
    ...
    res.setHeader("Transfer-Encoding", "chunked");
           ...
           if (!sessionId || typeof sessionId !== "string") {
                 try {
                         ...
                         res.write(full.split(HANDOFF_MARKER).join("").trim());
                         res.end();
                 } catch (err) {
      ...
      // If the bot couldn't fully answer, strip the marker, let the visitor know we're connecting
      // them with sales, and automatically flip the conversation into WhatsApp handoff mode.
      const needsHandoff = full.includes(HANDOFF_MARKER);
                 const cleaned = full.split(HANDOFF_MARKER).join("").trim();
                 const reply = needsHandoff
                   ? `${cleaned}\n\nLet me connect you with our sales team on WhatsApp — they'll be able to help with this.`
                         : cleaned;

      res.write(reply);
                 res.end();
                 await recordBotReply(conversation.id, reply);

      if (needsHandoff) {
              try {
                        await startHandoff(conversation.sessionId);
              } catch (err) {
                        console.error("Auto-handoff error:", err);
              }
      }
           } catch (err) {
                 console.error("Chat error:", err);
