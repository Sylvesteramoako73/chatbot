...
async function setConversationStatus(conversationId: string, status: "bot" | "handoff"): Promise<void> {
...
export async function startHandoff(sessionId: string): Promise<void> {
  ...
  // Only flip the conversation into handoff mode once we've actually managed to notify the
  // owner. Otherwise (e.g. an expired WhatsApp token) the visitor gets permanently stuck in a
  // broken conversation that can never reach the bot again.
  const wamid = await notifyOwner(notification);
    await setConversationStatus(conversation.id, "handoff");
...
export async function relayVisitorMessage(conversationId: string, text: string): Promise<void> {
    ...
    try {
          ...
          // WhatsApp relay is broken (e.g. expired token) — don't leave the visitor stuck forever.
      // Drop back to bot mode so subsequent messages get answered normally instead of erroring out.
      await setConversationStatus(conversationId, "bot");
          throw err;
    }
}
